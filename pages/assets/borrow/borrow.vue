						...
						<view class="order" v-for="(item, index) in dataList" :key="item.id">
							...
							<view class="item">
								...
								<view class="content">
									...
									<view class="type">借用人:{{ item.lprocessusername }}</view>
									<view class="type">借用时间:{{ item.busdatestr }}</view>
									<view class="type">预计归还时间:{{ item.returndatestr }}</view>
									<view class="delivery-time">创建时间:{{ item.create_time }}</view>
								</view>
							</view>
							<view class="bottom">
<!-- 								<view class="evaluate btn" @click="detail(item.id)">单据详情</view> -->
		...
		<view class="caozuo">	
			<u-button @click="openAddPage('/pages/assets/borrow/add/add')" style='width:50%' shape="circle" :ripple="true" type="primary">新增借用</u-button>
		</view>
	</view>
</template>
<script>
	export default {
		...
		methods: {
			...
			loadData:function(index){
				this.$u.post('/api/zc/resLoanreturn/ext/myList.do', {
					bustype:"JY",statustype:this.tabs[index].code
				}).then(res => {
					if(res.success){
