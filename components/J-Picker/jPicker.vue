<template>
	<view class="jPicker">
		<view @click="showPicker" class="showLine">
			<view style="text-align: right;"  v-if="listData.length>0&&nSel!=-1">
				{{listData[nSel][showKey]||listData[nSel]}}
			</view>
			<view style="color:#7d7d7d;text-align: right;" input-align="right" v-else>
				未选择
			</view>
		</view>
...
<script>
	 ...
	 * @property {String} showSearch 显示搜索框
	 ...
	 * @example <jPicker :disabled="false" :showSearch="true" class="cont" @sure="bindPickerChange($event,'TYPE')" showKey="Name" valKey="Value" :val="CurrentType" :options="FilterArray" />
	 */  
	export default {
		name: 'jPicker',
		data() {
		...
		props: ["options", "showKey", "valKey", "val", "position", "showSearch","disabled", "bgColor", "sureColor"],
